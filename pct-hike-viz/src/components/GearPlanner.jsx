...
const getItemWeightMeta = (item) => {
  ...
  const eachLbs = Number.isFinite(item?.weightEachLbs)
    ? item.weightEachLbs
  ...
  if (eachLbs > 0) {
    ...
    return {
      ...
      label:
        qty > 1
          ? `${formatWeightLabel(eachLbs)} × ${qty} = ${formatWeightLabel(
              total
            )}`
          : formatWeightLabel(total),
    };
  }
...
const getSourcesFromIds = (ids = []) =>
  ids.map((id) => resourcesById[id]).filter(Boolean);
      ...
      {sources.map((source) => {
        ...
        return source.url ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`gear-citation-chip gear-citation-chip--${
...
const normalizeCustomItem = (row) => ({
  ...
  weight:
    row.weight_label || (row.weight_val ? `${row.weight_val} lb` : "0 lb"),
...
function GearPlanner({ data, currentUser }) {
  ...
  useEffect(() => {
    ...
    const inflateLoadouts = (rows = []) => {
      ...
      return {
        ...
        gunnar: seen.has("gunnar")
          ? next.gunnar
          : new Set(defaultLoadouts.gunnar),
      };
    };
    ...
    channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "gear_loadouts" },
        (payload) => {
  ...
  useEffect(() => {
    ...
    channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "custom_items" },
        (payload) => {
          const eventType = payload.eventType || payload.type;
          if (eventType === "DELETE") {
  ...
  const persistLoadout = async (nextSet, hikerId, previousSet) => {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error("Gear save timed out after 10 seconds")),
        10000
      );
    });
  ...
  const equippedItems = inventory.filter((item) =>
    currentLoadoutIds.has(item.id)
  );
  const baseWeightGoal = Number(
    activeHiker.baseWeightGoal || data?.baseWeightGoalLbs || 0
  );
  ...
  const weightStatus =
    baseWeightGoal > 0 && carriedWeight > baseWeightGoal ? "warn" : "good";
  ...
  const getDisplaySpecs = (item) => {
    ...
    if (
      bucket === "worn" &&
      !base.some((s) => String(s).toLowerCase().includes("worn"))
    )
      return ["Worn", ...base];
    if (
      bucket === "consumable" &&
      !base.some((s) => String(s).toLowerCase().includes("consumable"))
    )
      return ["Consumable", ...base];
    return base;
  };
    ...
    <div className="gear-planner">
          ...
          {HIKERS.map((hiker) => {
            ...
            return (
              <button
                key={hiker.id}
                type="button"
                className={`hiker-tab ddg-hiker-tab ${
                  isActive ? "is-active" : ""
                }`}
              ...
              >
                ...
                <div className="hiker-weight-badge">
                  <span
                    className={
                      baseWeightGoal > 0 && loadoutWeight > baseWeightGoal
                        ? "weight-warn"
                        : "weight-good"
                    }
                  >
      ...
      <div className="gear-summary-grid">
        <div className="summary-card">
          ...
          <div className={`summary-value status-${weightStatus}`}>
        ...
        <div className="summary-card">
          ...
          <div className="summary-sub">
            Worn {wornWeight.toFixed(1)} lb • Consumables{" "}
      ...
      {isSyncing && (
        <p className="note" aria-live="polite">
          Syncing loadouts with mission control…
        </p>
      )}
      {syncError && (
